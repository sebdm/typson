...
(function(definition) {
    ...
    if (typeof exports === "object") {
        module.exports = definition(require("underscore"), require("q"), null, null, require("fs"), require("path"));
        // RequireJS
    } else if (typeof define === "function" && define.amd) {
        return define(["vendor/underscore", "vendor/q", "vendor/superagent", "vendor/typescriptServices", "fs",
            "path"], definition);
    }
})(function(underscore, Q, request, TypeScript, fs, path) {
    if (underscore) {
        _ = underscore;
    }
    var api = {};
    ...
    function inlineContent(filepath, fileStr) {
        fileStr = fileStr.replace(/\{\{#\s*([^ ]+?)\s*#\}\}/g, function(whole, first) {
            try {
                var contents = fs.readFileSync(path.join(path.dirname(filepath), first)).toString();
                contents = escapeContent(contents);
                return contents;
            } catch (ex) {
                return whole;
            }
